...
class skinDatasetFolder(data.Dataset):
    ...
    def __init__(self, train=True, iterNo=1, data_dir='../data'):
        ...
        self.train = train
        if self.train:
            # self.labels = torch.LongTensor([dta[1] for dta in self.train_data])
            self.labels = [dta[1] for dta in self.train_data]
            self.img_num = [self.labels.count(i) for i in range(self.cls_num)]

        else:
            # self.labels = torch.LongTensor([dta[1] for dta in self.test_data])
            self.labels = [dta[1] for dta in self.test_data]
        ...
        # if self.train:
        #     self.labels = [x[1] for x in self.train_data]
        # else:
        #     self.labels = [x[1] for x in self.test_data]
